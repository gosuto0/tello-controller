...
class UI(tk.Frame):
    ...
    def setup(self):
        ...
        self.log.debug("thread created -> tello")
        threading.Thread(target=lambda: self.rc_input(),args=()).start()
        self.log.debug("thread created -> rc_input")
    ...
    def setup_frame(self):  
        ...
        self.log.debug("create video frame (1280x720)")
...
#agz	z加速度
        ...
        Label(self.controller_frame,textvariable=self.bat,bg=self.theme,fg="white").place(x=10,y=290)
        Label(self.controller_frame,textvariable=self.time,bg=self.theme,fg="white").place(x=10,y=320)
        Label(self.controller_frame,textvariable=self.temp,bg=self.theme,fg="white").place(x=10,y=350)
        Label(self.controller_frame,textvariable=self.tof,bg=self.theme,fg="white").place(x=10,y=380)
        #BTNfunc
        
        Button(self.controller_frame, text="速度変更", bg=self.button_theme, relief="raised", width=5, height=2, justify=CENTER, command=lambda : self.tello_class.send_data("switch_speed")).place(x=10, y=30)
        Button(self.controller_frame, text="Emr", bg=self.button_theme, relief="raised", width=5, height=2, justify=CENTER, command=lambda : self.tello_class.send_data("emergency")).place(x=10, y=80)
        Button(self.controller_frame, text="VideoOn", bg=self.button_theme, relief="raised", width=5, height=2, justify=CENTER, command=lambda : self.tello_class.send_data("streamon")).place(x=10, y=130)
        Button(self.controller_frame, text="Off", bg=self.button_theme, relief="raised", width=5, height=2, justify=CENTER, command=lambda : self.tello_class.send_data("streamoff")).place(x=10, y=180)
        Button(self.controller_frame, text="Tgl", bg=self.button_theme, relief="raised", width=5, height=2, justify=CENTER, command=lambda : self.toggle_videomode()).place(x=10, y=230)

        #CBfunc
        tk.Checkbutton(self.controller_frame, text="キーボード入力", bg=self.theme, fg="white", selectcolor=self.theme, command=lambda:self.switch_bind(), variable=self.key_input).place(x=5, y=3)

        #scalebar
        self.drone_v = tk.Scale(self.controller_frame,from_=100,to=0,label="移動量",bg=self.button_theme,length=230)
    ...
    def overray_image(self, frame):
        ...
        if self.videomode == 0:
            cv2.putText(cv_image, "VideoMode: None", (3,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1)
        if self.videomode == 1:
            cv_image = face.face_detect(cv_image)
            cv2.putText(cv_image, "VideoMode: CascadeFace", (3,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1)
        if self.videomode == 2:
            cv_image = yolo.main_n(frame,cv_image)
            cv2.putText(cv_image, "VideoMode: Yolov5", (3,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1)
        if self.videomode == 3:
            cv_image = yolo.main_p(frame,cv_image)
            cv2.putText(cv_image, "VideoMode: Yolov5Pandas[Person]", (3,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1)
        if self.videomode == 4:
            cv_image = yolo.main_m(frame,cv_image)
            cv2.putText(cv_image, "VideoMode: Yolov5Pandas[All]", (3,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1)
...
def main():
    global value
    root = tk.Tk()
    threading.Thread(target=lambda:tello.main(tello_ip, tello_port, video_ip),args=()).start()
    while value == None: pass
