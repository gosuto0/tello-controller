...
def main_n(frame,cv_image):
    ...
    for data in datas:
        if data[4] > 50 / 100:
            ...
            cv_image = cv2.rectangle(cv_image, (left, top), (right, bottom), (0, 255, 0), 3)
    ...
    for data in datas:
        if data[5] == "person":
            if data[4] > 50 / 100:
                ...
                cv2.rectangle(cv_image, (left, top), (right, bottom), class_colors[class_names.index(data[5])], 2)
                cv2.putText(cv_image, text, (left, top), cv2.FONT_HERSHEY_SIMPLEX, 1, class_colors[class_names.index(data[5])], 2)
    return cv_image
